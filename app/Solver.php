...
class Solver {
	...
	private $numOfPlayersPerPosition;
	private $randNumPerPosition;
	private $validTotalSalary;
	private $totalSalary;
	...
	public function buildLineupsWithTopPlays($players) {
		...
		for ($i=0; $i < 10000; $i++) { 
			$lineup = $this->buildOneLineupWithTopPlays($players, $numOfPlayersPerPosition);
		}
	}

	private buildOneLineupWithTopPlays($players, $numOfPlayersPerPosition) {
	...
	private function validateTotalSalary($lineup) {
		$totalSalary = 0;

		foreach ($lineup as $rosterSpot) {
			$totalSalary += $rosterSpot->salary;
		}
